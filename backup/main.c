...
/* USER CODE BEGIN PFP */
void HandleHapticFeedback(uint8_t feedback) {
  if (feedback > 0) {
	  // Turn on blue led
...
int main(void)
{
  ...
  while (1)
  {
    ...
    HandleHapticFeedback(game_controller.haptic_feedback);
    USBD_HID_SendReport(&hUsbDeviceFS, (uint8_t *)&game_controller, 6);
    /* USER CODE END WHILE */
